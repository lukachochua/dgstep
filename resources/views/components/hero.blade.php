<section
    class="relative min-h-screen pt-20 bg-gradient-to-r from-[#0b0f1a] via-[#141d2f] to-[#0b0f1a] text-white overflow-hidden flex items-center z-0">
    <!-- Background Layer / Illustration -->
    <div class="absolute inset-0 z-0">
        <img src="https://images.unsplash.com/photo-1499428665502-503f6c608263?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="" class="w-full h-full object-cover opacity-10 mix-blend-lighten" />
    </div>

    <div
        class="container mx-auto px-4 sm:px-6 md:px-8 z-10 relative flex flex-col md:flex-row items-center gap-16 select-none">
        <!-- Left Column: Text Content -->
        <div class="max-w-2xl text-left">
            <h1 class="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight drop-shadow-lg">
                Stop Wasting Time on Paperwork —
                <span class="text-[var(--color-electric-sky)]">Digitize Your Pawnshop or Business with DG STEP</span>
            </h1>

            <p class="mt-4 text-lg md:text-xl max-w-lg text-white/80 drop-shadow-sm">
                From pawnshops to service businesses — we build fast, reliable software that simplifies operations,
                automates records, and helps you grow.
            </p>

            <div class="mt-6 flex gap-4">
                <a href="#contact"
                    class="inline-block border-2 border-white text-white font-semibold px-6 py-3 rounded-lg hover:bg-white hover:text-[var(--color-electric-sky)] transition select-auto">
                    Contact Us
                </a>
            </div>
        </div>
    </div>

    {{-- <!-- Right Column: .Scroll Indicator / Secondary Content -->
        <div class="relative hidden md:block w-full max-w-sm">

        </div> --}}
    </div>
</section>
