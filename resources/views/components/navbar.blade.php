...
<div
  x-data="{
    ...
    isHiding: false,      // animate only when hiding
    lastY: 0,
    lastTS: 0,
    lastRevealY: 0,
    open: false,
    ...
    /* Behavior knobs */
    topSnap: 2,               // snap-visible ONLY when <= this px from very top
    hideTolerance: 16,        // px jitter guard
    minHideVelocity: 0.05,    // px/ms (~50 px/s) to consider 'going down'
    hideOffsetDesktop: 240,   // must scroll this far past last reveal before hiding
    ...
    resolveHideOffset(){
      ...
      this.hideOffset = mobile ? this.hideOffsetMobile : this.hideOffsetDesktop;
    },
    ...
    applyVisibility(nextY, ts){
      ...
      // Instant snap visible only at the very top
      ...
      // Hide only after we've gone far enough past our last reveal point
      const farEnough = (nextY - this.lastRevealY) >= this.hideOffset;

      if (goingDown && farEnough) {
        this.isHiding = true;     // animate (pronounced) slide-up
        this.isVisible = false;
      } else if (goingUp) {
        // stay hidden while going up; only snap back at very top
      }
    },
    ...
    setup(){
      ...
      // Keep visible while mobile menu is open; lock scroll; re-measure gutter
      this.$watch('open', (v) => {
        this.lockScroll(v);
        if (v) {
          this.isHiding = false; this.isVisible = true;
...
>
  ...
  <nav
    ...
    class="fixed top-0 left-0 z-50
           ...
           border-b shadow-sm supports-[backdrop-filter]:backdrop-blur-lg"
    style="height: var(--nav-top-h); right: 0;"
    :style="{
      /* MUCH more pronounced, slow, smooth hide: bigger translate + fade + long duration + smooth easing */
      transform: isVisible ? 'translateY(0)' : 'translateY(-200%)',
      opacity:   isVisible ? 1 : 0,
      transitionProperty: 'transform, opacity, box-shadow, border-color',
      transitionDuration: isHiding ? '820ms' : '0ms',
      transitionTimingFunction: 'cubic-bezier(.16,1,.3,1)',

      /* Always same surface (top and scrolled): tinted in light, translucent dark */
      background: (theme === 'dark')
        ? 'rgba(24,24,27,0.60)' // dark
        : 'color-mix(in oklab, var(--nav-bottom-bg, #726EDB) 85%, transparent)', // ~15% transparency
      borderColor: (theme === 'dark') ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.12)',
    ...
    <div class="mx-auto w-full max-w-[var(--container-content)] flex items-center justify-between">
      ...
      <div class="flex items-center gap-1.5 md:gap-2">
        ...
        <button
          ...
          class="hidden md:inline-flex items-center justify-center h-9 w-9 rounded-full focus-ring hover:shadow-sm transition"
        ...
        <button
          type="button"
          class="relative inline-flex items-center justify-center h-9 w-9 rounded-full focus-ring hover:shadow-sm transition"
          :aria-label="`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`"
          role="switch"
          :aria-checked="(theme === 'dark').toString()"
          @click="toggleTheme()"
        >
          <svg x-cloak class="theme-icon icon--sun absolute inset-0 m-auto h-5 w-5 pointer-events-none"
               viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"
               stroke-linecap="round" stroke-linejoin="round">
            <circle cx="12" cy="12" r="4"/>
            <path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/>
          </svg>
          <svg x-cloak class="theme-icon icon--moon absolute inset-0 m-auto h-5 w-5 pointer-events-none"
               viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"
               stroke-linecap="round" stroke-linejoin="round">
    ...
    <div class="mx-auto max-w-[var(--container-content)] px-4 sm:px-6 md:px-8 py-4
      ...
      <div class="flex justify-center items-center gap-3 pt-1">
        ...
        <button
          ...
          @click="toggleTheme()"
        >
          <svg x-cloak class="theme-icon icon--sun absolute inset-0 m-auto h-6 w-6 pointer-events-none"
               viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"
               stroke-linecap="round" stroke-linejoin="round">
          ...
          <svg x-cloak class="theme-icon icon--moon absolute inset-0 m-auto h-6 w-6 pointer-events-none"
               viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"
               stroke-linecap="round" stroke-linejoin="round">
